...
.finish:
	...
	err_char_in_num		db "Hata: unexpected char in number", 0
	err_cmd_unknown		db "Hata: unknown command", 0
	err_divide_by_zero	db "Hata: attempt to divide by zero", 0
	err_doloop_maximum	db "Hata: DO/LOOP nesting limit exceeded", 0
	err_file_notfound	db "Hata: file not found", 0
	err_goto_notlabel	db "Hata: GOTO or GOSUB not followed by label", 0
	err_label_notfound	db "Hata: label not found", 0
	err_nest_limit		db "Hata: FOR or GOSUB nest limit exceeded", 0
	err_next		db "Hata: NEXT without FOR", 0
	err_loop		db "Hata: LOOP without DO", 0
	err_print_type		db "Hata: PRINT not followed by quoted text or variable", 0
	err_quote_term		db "Hata: quoted string or char not terminated correctly", 0
	err_return		db "Hata: RETURN without GOSUB", 0
	err_string_range	db "Hata: string location out of range", 0
	err_syntax		db "Hata: syntax error", 0
